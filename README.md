...
Исходя из данных видно, что TreeMap так же, как и TreeSet обходит аналоги на операции добавления большого элементов, в то время, как удаление элемента выполняется за примерно одинаковое время. LinkedHashMap также работает несколько медленнее из-за необходимости записи ссылок на предыдущий и последущий элементы.
		
						Часть 4. Результаты и выводы

В ходе лабораторной работы были выполнены все поставленные задачи, реализован собственный класс LinkedList, проведено сравнение производительности указанных коллекций, углублены знания в Collection Framework. По каждому из пунктов были сделаны следующие выводы:

1) Реализация MyLinkedList оказалось сопастовима по производительности со стандартной коллекцей, методы были реализованы похожим образом, собственную коллекцию также можно использовать для хранения объектов любого типа.
2) Стандартный LinkedList опережает ArrayList в операциях вставки, однако сильно отстает в поиске по индексу. Так как реализация ArrayList оптимизироана в настоящей версии java (16.0), операция добавления элемента в середину коллекции была быстрее у ArrayList, несмотря на необходимость перекопирования массива. С крайними элементами LinkedList работает на порядок быстрее (это касается, как добавления, так и удалению элементов).
3) Коллекция LinkedHashSet работает медленнее своих аналогов HashSet и TreeSet при операции добавления, так как она вынуждена сохранять ссылку на смежные элементы, однако это предоставляет возможность обращаться к элементам по индексу (сохраняет порядок добавления). Если данный функционал не является необходимым, лучше использовать HashMap. Также интересен тот факт, что TreeSet опередила аналоги при добавлении большого количества элементов, вероятно это связано с большой скоростью поиска по коллекции (проверка на уникальность).
4) Ситуация с имплементациями интерфейса Map схожа - TreeMap по некоторым причинам оказался быстрее всех в добавлении элементов, LinkedHashMap оказался медленнее всех. Удаление элементов выполнилась за примерно одинаковое время.
